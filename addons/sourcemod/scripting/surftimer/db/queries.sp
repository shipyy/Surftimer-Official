...
//ck_playervotes
char sql_getattributes[] = "SELECT AVG(tier), AVG(longevity), AVG(difficulty), AVG(textures), AVG(fun), AVG(unit), AVG(tech), AVG(bonuses) FROM ck_playervotes WHERE mapname ='%s';";
char sql_PlayerAttributes[] = "SELECT * FROM ck_playervotes WHERE steamid = '%s' AND mapname ='%s';";
char sql_create_playervotes[] = "CREATE TABLE IF NOT EXISTS ck_playervotes (steamid VARCHAR(32), mapname VARCHAR(32), date TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, longevity int(12) NOT NULL DEFAULT '0', tier int(12) NOT NULL DEFAULT '0', difficulty int(12) NOT NULL DEFAULT '0', textures int(12) NOT NULL DEFAULT '0', fun int(12) NOT NULL DEFAULT '0', unit int(12) NOT NULL DEFAULT '0', tech int(12) NOT NULL DEFAULT '0', bonuses int(12) NOT NULL DEFAULT '0',PRIMARY KEY(steamid, mapname))DEFAULT CHARSET=utf8mb4;";
char sql_InsertPlayerVote[] = "INSERT INTO ck_playervotes (tier, longevity, difficulty, textures, fun, unit, tech, bonuses, steamid, mapname) VALUES ('%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%s', '%s');";
char sql_UpdatePlayerVote[] = "UPDATE ck_playervotes SET tier = '%d', longevity = '%d', difficulty = '%d', textures = '%d', fun = '%d', unit = '%d', tech = '%d', bonuses = '%d' WHERE steamid = '%s' AND mapname = '%s';";

// ck_playeroptions2
char sql_createPlayerOptions[] = "CREATE TABLE `ck_playeroptions2` (`steamid` varchar(32) NOT NULL DEFAULT '', `timer` int(11) NOT NULL DEFAULT '1', `hide` int(11) NOT NULL DEFAULT '0', `chat` int(11) NOT NULL DEFAULT '0', `viewmodel` int(11) NOT NULL DEFAULT '1', `autobhop` int(11) NOT NULL DEFAULT '1', `checkpoints` int(11) NOT NULL DEFAULT '1', `gradient` int(11) NOT NULL DEFAULT '3', `speedmode` int(11) NOT NULL DEFAULT '0', `centrespeed` int(11) NOT NULL DEFAULT '0', `centrehud` int(11) NOT NULL DEFAULT '1', teleside int(11) NOT NULL DEFAULT '0', `module1c` int(11) NOT NULL DEFAULT '1', `module2c` int(11) NOT NULL DEFAULT '2', `module3c` int(11) NOT NULL DEFAULT '3', `module4c` int(11) NOT NULL DEFAULT '4', `module5c` int(11) NOT NULL DEFAULT '5', `module6c` int(11) NOT NULL DEFAULT '6', `sidehud` int(11) NOT NULL DEFAULT '1', `module1s` int(11) NOT NULL DEFAULT '5', `module2s` int(11) NOT NULL DEFAULT '0', `module3s` int(11) NOT NULL DEFAULT '0', `module4s` int(11) NOT NULL DEFAULT '0', `module5s` int(11) NOT NULL DEFAULT '0', prestrafe int(11) NOT NULL DEFAULT '0', cpmessages int(11) NOT NULL DEFAULT '1', wrcpmessages int(11) NOT NULL DEFAULT '1', hints int(11) NOT NULL DEFAULT '1', showtime int(11) NOT NULL DEFAULT '1' , csd_update_rate int(11) NOT NULL DEFAULT '1' , csd_pos_x float(11) NOT NULL DEFAULT '0.5' , csd_pos_y float(11) NOT NULL DEFAULT '0.3' , csd_r int(11) NOT NULL DEFAULT '255', csd_g int(11) NOT NULL DEFAULT '255', csd_b int(11) NOT NULL DEFAULT '255', PRIMARY KEY (`steamid`)) DEFAULT CHARSET=utf8mb4;";
char sql_insertPlayerOptions[] = "INSERT INTO ck_playeroptions2 (steamid) VALUES ('%s');";
char sql_selectPlayerOptions[] = "SELECT timer, hide, chat, viewmodel, autobhop, checkpoints, gradient, speedmode, centrespeed, centrehud, teleside, module1c, module2c, module3c, module4c, module5c, module6c, sidehud, module1s, module2s, module3s, module4s, module5s, prestrafe, prespeedmode, cpmessages, wrcpmessages, hints, showtime, csd_update_rate, csd_pos_x, csd_pos_y, csd_r, csd_g, csd_b, custom_type, default_style FROM ck_playeroptions2 where steamid = '%s';";
char sql_updatePlayerOptions[] = "UPDATE ck_playeroptions2 SET timer = %i, hide = %i, chat = %i, viewmodel = %i, autobhop = %i, checkpoints = %i, gradient = %i, speedmode = %i, centrespeed = %i, centrehud = %i, teleside = %i, module1c = %i, module2c = %i, module3c = %i, module4c = %i, module5c = %i, module6c = %i, sidehud = %i, module1s = %i, module2s = %i, module3s = %i, module4s = %i, module5s = %i, prestrafe = %i, prespeedmode = %i, cpmessages = %i, wrcpmessages = %i, hints = %i, showtime = %i, csd_update_rate = %i, csd_pos_x = %f, csd_pos_y = %f, csd_r= %i, csd_g = %i, csd_b = %i, custom_type = %i, default_style = %i where steamid = '%s'";
...
char sql_insertPlayerRank[] = "INSERT INTO ck_playerrank (steamid, steamid64, name, country, countryCode, joined, style) VALUES('%s', '%s', '%s', '%s', '%s', %i, %i)";
char sql_updatePlayerRankPoints[] = "UPDATE ck_playerrank SET name ='%s', points ='%i', wrpoints = %i, wrbpoints = %i, wrcppoints = %i, top10points = %i, groupspoints = %i, mappoints = %i, bonuspoints = %i, finishedmapspro='%i', finishedbonuses = %i, finishedstages = %i, wrs = %i, wrbs = %i, wrcps = %i, top10s = %i, `groups` = %i, avgmaprank = %i where steamid='%s' AND style = %i;";
char sql_updatePlayerRankPoints2[] = "UPDATE ck_playerrank SET name ='%s', points ='%i', wrpoints = %i, wrbpoints = %i, wrcppoints = %i, top10points = %i, groupspoints = %i, mappoints = %i, bonuspoints = %i, finishedmapspro='%i', finishedbonuses = %i, finishedstages = %i, wrs = %i, wrbs = %i, wrcps = %i, top10s = %i, `groups` = %i, avgmaprank = %i, country = '%s', countryCode = '%s' where steamid='%s' AND style = %i;";
...
char sql_insertPlayerTmp[] = "INSERT INTO ck_playertemp (cords1, cords2, cords3, angle1,angle2,angle3,runtimeTmp,steamid,mapname,EncTickrate,Stage,zonegroup, CCP_times, CCP_attempts ) VALUES ('%f','%f','%f','%f','%f','%f','%f','%s', '%s', '%i', %i, %i, '%s', '%s');";
char sql_updatePlayerTmp[] = "UPDATE ck_playertemp SET cords1 = '%f', cords2 = '%f', cords3 = '%f', angle1 = '%f', angle2 = '%f', angle3 = '%f', runtimeTmp = '%f', mapname ='%s', EncTickrate='%i', Stage = %i, zonegroup = %i, CCP_times = '%s', CCP_attempts = '%s' WHERE steamid = '%s';";
char sql_deletePlayerTmp[] = "DELETE FROM ck_playertemp where steamid = '%s';";
char sql_selectPlayerTmp[] = "SELECT cords1,cords2,cords3, angle1, angle2, angle3,runtimeTmp, EncTickrate, Stage, zonegroup, CCP_times, CCP_attempts FROM ck_playertemp WHERE steamid = '%s' AND mapname = '%s';";
...
char sql_selectReplayCPTicks[] = "SELECT cp, frame, style FROM ck_replays WHERE mapname = '%s' AND style = '%i' ORDER BY cp ASC;";
...
//ck_options_sound
char sql_createSoundOptions[] = "CREATE TABLE IF NOT EXISTS ck_soundoptions (steamid VARCHAR(32), enabled int(12) NOT NULL DEFAULT '1', map_wr int(12) NOT NULL DEFAULT '13', map_pb int(12) NOT NULL DEFAULT '10', map_top10 int(12) NOT NULL DEFAULT '11', bonus_wr int(12) NOT NULL DEFAULT '13', bonus_pb int(12) NOT NULL DEFAULT '10', bonus_top10 int(12) NOT NULL DEFAULT '11', stage_wr int(12) NOT NULL DEFAULT '4', timerstart int(12) NOT NULL DEFAULT '2', missed_pb int(12) NOT NULL DEFAULT '1', error int(12) NOT NULL DEFAULT '3', PRIMARY KEY(steamid)) DEFAULT CHARSET=utf8mb4;";
char sql_selectSoundOptions[] = "SELECT enabled, map_wr, map_pb, map_top10, bonus_wr, bonus_pb, bonus_top10, stage_wr, timerstart, missed_pb, error FROM ck_soundoptions WHERE steamid = '%s';";
char sql_insertSoundOptions[] = "INSERT INTO ck_soundoptions (steamid) VALUES ('%s');";
char sql_updateSoundOptions[] = "UPDATE ck_soundoptions SET enabled = '%i', map_wr = '%i', map_pb = '%i', map_top10 = '%i', bonus_wr = '%i', bonus_pb = '%i', bonus_top10 = '%i', stage_wr = '%i', timerstart = '%i', missed_pb = '%i', error = '%i' WHERE steamid='%s';";