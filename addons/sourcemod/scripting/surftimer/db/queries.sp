...
char sql_selectCheckpointsTimesType[] = "SELECT cp1, cp2, cp3, cp4, cp5, cp6, cp7, cp8, cp9, cp10, cp11, cp12, cp13, cp14, cp15, cp16, cp17, cp18, cp19, cp20, cp21, cp22, cp23, cp24, cp25, cp26, cp27, cp28, cp29, cp30, cp31, cp32, cp33, cp34, cp35 FROM ck_checkpoints WHERE mapname = '%s' AND steamid = (SELECT steamid FROM ck_playertimes WHERE mapname = '%s' AND style = '0' ORDER BY runtimepro LIMIT %i,1);";
char sql_selectCustomCheckpointsTypeRecords_Map[] = "SELECT runtimepro FROM ck_playertimes WHERE mapname = '%s' AND style = '0' ORDER BY runtimepro ASC LIMIT %i,1;";
char sql_selectCustomCheckpointsTypeRecords_Bonus[] = "SELECT runtime, zonegroup FROM ck_bonus WHERE mapname = '%s' AND zonegroup = '%i' AND style = '0' ORDER BY runtime ASC LIMIT %i,1;";
char sql_selectCheckpointsSpeedsType[] = "SELECT cp1speed, cp2speed, cp3speed, cp4speed, cp5speed, cp6speed, cp7speed, cp8speed, cp9speed, cp10speed, cp11speed, cp12speed, cp13speed, cp14speed, cp15speed, cp16speed, cp17speed, cp18speed, cp19speed, cp20speed, cp21speed, cp22speed, cp23speed, cp24speed, cp25speed, cp26speed, cp27speed, cp28speed, cp29speed, cp30speed, cp31speed, cp32speed, cp33speed, cp34speed, cp35speed FROM ck_checkpoints WHERE mapname = '%s' AND steamid = (SELECT steamid FROM ck_playertimes WHERE mapname = '%s' AND style = '0' ORDER BY runtimepro LIMIT %i,1);";
...
char sql_selectPlayerOptions[] = "SELECT timer, hide, sounds, chat, viewmodel, autobhop, checkpoints, gradient, speedmode, centrespeed, centrehud, teleside, module1c, module2c, module3c, module4c, module5c, module6c, sidehud, module1s, module2s, module3s, module4s, module5s, prestrafe, cpmessages, wrcpmessages, hints, csd_update_rate, csd_pos_x, csd_pos_y, csd_r, csd_g, csd_b, comparetype FROM ck_playeroptions2 where steamid = '%s';";
char sql_updatePlayerOptions[] = "UPDATE ck_playeroptions2 SET timer = %i, hide = %i, sounds = %i, chat = %i, viewmodel = %i, autobhop = %i, checkpoints = %i, gradient = %i, speedmode = %i, centrespeed = %i, centrehud = %i, teleside = %i, module1c = %i, module2c = %i, module3c = %i, module4c = %i, module5c = %i, module6c = %i, sidehud = %i, module1s = %i, module2s = %i, module3s = %i, module4s = %i, module5s = %i, prestrafe = %i, cpmessages = %i, wrcpmessages = %i, hints = %i, csd_update_rate = %i, csd_pos_x = %f, csd_pos_y = %f, csd_r= %i, csd_g = %i, csd_b = %i, comparetype = %i where steamid = '%s'";
