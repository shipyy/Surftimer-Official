...
char sql_selectPlayerProfile[] = "SELECT steamid, steamid64, name, country, points, wrpoints, wrbpoints, wrcppoints, top10points, groupspoints, mappoints, bonuspoints, finishedmapspro, finishedbonuses, finishedstages, wrs, wrbs, wrcps, top10s, `groups`, lastseen, avgmaprank FROM ck_playerrank WHERE steamid = '%s' AND style = '%i';";
char sql_AVGMapRank[] = "SELECT AVG(map_rank) FROM (SELECT mapname as map, (SELECT COUNT(*) FROM ck_playertimes WHERE runtimepro <= (SELECT runtimepro FROM ck_playertimes WHERE steamid = '%s' AND mapname = map AND runtimepro > -1.0 AND style = 0) AND mapname = map AND style = 0 AND runtimepro > -1.0 ORDER BY runtimepro) as map_rank FROM ck_playertimes WHERE steamid = '%s' AND style = 0) as map_ranks_tables;";
