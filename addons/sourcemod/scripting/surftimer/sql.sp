		...
		if(key == 1)
		{
			...
			switch(g_SelectedEditOption[client])
			{
				...
				case 2:
				{
					...
					SQL_TQuery(g_hDb, SQL_CheckCallback, BonusPRruntime, _, DBPrio_Low);
...
public void CalculatePlayerRank(int client, int style)
{
	...
	g_AVGMapRank[client][style] = 0; // Average Map Rank
...
public void sql_CalcuatePlayerRankCallback(Handle owner, Handle hndl, const char[] error, any pack)
{
	...
	else
	{
		...
		if (client <= MaxClients)
		{
			...
			Format(szQuery, sizeof(szQuery), sql_insertPlayerRank, szSteamId, szSteamId64, szName, g_szCountry[client], g_szCountryCode[client], GetTime(), style);
...
public void sql_CountFinishedMapsCallback(Handle owner, Handle hndl, const char[] error, any pack)
{
	...
	int client = ReadPackCell(pack);
	int style = ReadPackCell(pack);
	CloseHandle(pack); // BOOKMARK FOR POSSIBLE REVERT
...
public void sql_AVGMapRankCallback(Handle owner, Handle hndl, const char[] error, any pack)
{
	if (hndl == null)
	{
		LogError("[SurfTimer] SQL Error (sql_AVGMapRankCallback): %s", error);
		CloseHandle(pack);
		return;
	}
	...
	if (SQL_HasResultSet(hndl) && SQL_FetchRow(hndl))
	{
		g_AVGMapRank[client][style] = RoundToNearest(SQL_FetchFloat(hndl, 0));
	}
...
public void db_updatePoints(int client, int style)
{
	...
	else
	{
		if (IsValidClient(client))
		{
			...
			Format(szQuery, sizeof(szQuery), sql_updatePlayerRankPoints2, szName, g_pr_points[client][style], g_Points[client][style][3], g_Points[client][style][4], g_Points[client][style][6], g_Points[client][style][5], g_Points[client][style][2], g_Points[client][style][0], g_Points[client][style][1], g_pr_finishedmaps[client][style], g_pr_finishedbonuses[client][style], g_pr_finishedstages[client][style], g_WRs[client][style][0], g_WRs[client][style][1], g_WRs[client][style][2], g_Top10Maps[client][style], g_GroupMaps[client][style], g_AVGMapRank[client][style], g_szCountry[client], g_szCountryCode[client], szSteamId, style);
			SQL_TQuery(g_hDb, sql_updatePlayerRankPointsCallback, szQuery, pack, DBPrio_Low);
		}
	}
}

// 8. Calculations done, if calculating all, move forward, if not announce changes.
...
public void db_viewPlayerPointsCallback(Handle owner, Handle hndl, const char[] error, any client)
{
	...
	else
	{
		if (IsValidClient(client))
		{
			...
			Format(szQuery, sizeof(szQuery), sql_insertPlayerRank, g_szSteamID[client], szSteamId64, szName, g_szCountry[client], g_szCountryCode[client], GetTime());
			SQL_TQuery(g_hDb, SQL_CheckCallback, szQuery, _, DBPrio_Low);
...
{
	char szQuery[512];
	Format(g_pr_szrank[client], sizeof(szQuery), "");
...
public void sql_selectPlayerProfileCallback(Handle owner, Handle hndl, const char[] error, any pack)
{
	...
	if (SQL_HasResultSet(hndl) && SQL_FetchRow(hndl))
	{
		...
		int avg_map_rank = SQL_FetchInt(hndl, 20);
		...
		// Format Profile Menu
		char szCompleted[1024], szMapPoints[128], szBonusPoints[128], szTop10Points[128], szStagePc[128], szMiPc[128], szLastSeen[128], szAVGMapRank[128];
		...
		Format(szAVGMapRank, 128, "Average Map Rank: %i", avg_map_rank);

		Format(szCompleted, 1024, "Completed - Points (%s%c):\n%s\n%s\n%s\n%s\n \n%s\n \n%s\n \nRecords:", szTotalPerc, PERCENT, szMapPoints, szBonusPoints, szTop10Points, szStagePc, szMiPc, szAVGMapRank);
		...
		char szMapRecords[128], szStageRecords[128], szBonusRecords[128];
		Format(szMapRecords, 128,"Map Records: %i", wrs);
	...
	if (action == MenuAction_Select)
	{
		switch (item)
		{
			...
			case 1: {
				...
				RecordsMenu(client, 0, szSteamId); // MAP RECORDS
			}
			case 2: {
				char szSteamId[32];
				GetMenuItem(menu, item, szSteamId, 32);
				RecordsMenu(client, 1, szSteamId); // STAGE RECORDS
			}
			case 3: {
				char szSteamId[32];
				GetMenuItem(menu, item, szSteamId, 32);
				RecordsMenu(client, 2, szSteamId); // BONUS RECORDS
			}
			case 4:
			{
				char szSteamId[32];
...
public void RecordsMenu(int client, int option, char szSteamID[32])
{	
	Handle pack = CreateDataPack();
	WritePackCell(pack, client);
	WritePackString(pack, szSteamID);
...
public void sql_selectRecordsMenuCallback(Handle owner, Handle hndl, const char[] error, any data){

	if (hndl == null)
	{
		LogError("[SurfTimer] SQL Error (sql_selectRecordsMenuCallback): %s", error);
		CloseHandle(data);
		return;
	}
	...
	if (SQL_HasResultSet(hndl)){
		...
		Menu theRecordsMenu = CreateMenu(RecordsMenuHandler);
		...
		while(SQL_FetchRow(hndl)){
			...
			int option = SQL_FetchInt(hndl, 0);

			switch(option){
				//MAP RECORDS
				case 0: {
					SetMenuTitle(theRecordsMenu, "Map Records");
					...
					Format(szItem, sizeof(szItem), "%s - %s ", szMapName, szruntime);
					AddMenuItem(theRecordsMenu, "", szItem, ITEMDRAW_DISABLED);
				}
				// STAGE RECORDS
				case 1: {
					SetMenuTitle(theRecordsMenu, "Stage Records");
					...
					Format(szItem, sizeof(szItem), "%s - Stage %i - %s ", szMapName, stage, szruntime);
					AddMenuItem(theRecordsMenu, "", szItem, ITEMDRAW_DISABLED);
				}
				// BONUS RECORDS
				case 2: {
					SetMenuTitle(theRecordsMenu, "Bonus Records");
					SQL_FetchString(hndl, 1, szMapName, sizeof(szMapName));

					bonus_nr = SQL_FetchInt(hndl, 2);
					runtime = SQL_FetchFloat(hndl, 3);

					FormatTimeFloat(client, runtime, 3, szruntime, sizeof(szruntime));

					Format(szItem, sizeof(szItem), "%s - Bonus %i - %s ", szMapName, bonus_nr, szruntime);
					AddMenuItem(theRecordsMenu, "", szItem, ITEMDRAW_DISABLED);
				}
			}
		...
		SetMenuExitBackButton(theRecordsMenu, true);
		DisplayMenu(theRecordsMenu, client, MENU_TIME_FOREVER);
	}

}

public int RecordsMenuHandler(Handle menu, MenuAction action, int client, int item)
{
...
{
	char szQuery[1024];
	Format(szQuery, sizeof(szQuery), sql_selectTopSurfers, mapname, 0, 0);
	Handle pack = CreateDataPack();
	WritePackCell(pack, client);
...
public void db_selectMapTopSurfers(int client, char mapname[128])
{
	char szQuery[1024];
	int type = 0;
	Format(szQuery, sizeof(szQuery), sql_selectTopSurfers2, mapname);
	Handle pack = CreateDataPack();
	WritePackCell(pack, client);
	WritePackString(pack, mapname);
...
public void sql_selectTopSurfersCallback(Handle owner, Handle hndl, const char[] error, any data)
{
	...
	{
		float recordtime;
		char szFormattedTimeDifference[64];
		int i = 1;
		while (SQL_FetchRow(hndl))
		{
			...
			if (i == 1 || (i > 1 && StrEqual(szFirstMap, szMap)))
			{
				if (i < 51)
				{
					...
					//FORMAT RECORD TIME DIFFERENCE
					if ( i > 1 ) {
						FormatTimeFloat(client, recordtime - time, 3, szFormattedTimeDifference, sizeof szFormattedTimeDifference);
						Format(szFormattedTimeDifference, 64, "(+%s)", szFormattedTimeDifference);
					}
					else if ( i == 1 ) {
						Format(szFormattedTimeDifference, 64, "(+00:00:000)");
					}

					if (time < 3600.0)
						Format(szTime, 32, "   %s", szTime);
					if (i == 100)
						Format(szValue, 128, "[%i.] %s %s |	» %s", i, szTime, szFormattedTimeDifference, szName);
					if (i >= 10)
						Format(szValue, 128, "[%i.] %s %s |    » %s", i, szTime, szFormattedTimeDifference, szName);
					else
						Format(szValue, 128, "[0%i.] %s %s |    » %s", i, szTime, szFormattedTimeDifference, szName);
...
public void db_currentRunRank_Prac(int client, int style, int zgroup)
{
	if (!IsValidClient(client))
		return;
...
public void SQL_CurrentRunRank_PracCallback(Handle owner, Handle hndl, const char[] error, any data)
{
	if (hndl == null)
	{
		LogError("[SurfTimer] SQL Error (SQL_CurrentRunRankCallback): %s", error);
...
public void db_currentRunRank_StagePrac(int client, int style, int stage, float fClientPbStageTime)
{
	...
	char szQuery[1024];
	Format(szQuery, sizeof(szQuery), "SELECT count(runtimepro)+1 FROM ck_wrcps WHERE mapname = '%s' AND stage = '%i' AND style = '%i' AND runtimepro < '%f';", g_szMapName, stage, style, g_fCurrentRunTime[client]);
...
public void sql_selectRecordCallback(Handle owner, Handle hndl, const char[] error, any data)
{
	...
	{ // No record found from database - Let's insert
		...
		Format(szQuery, sizeof(szQuery), sql_insertPlayerTime, g_szSteamID[data], g_szMapName, szName, g_fFinalTime[data], 0, g_iPreStrafe[0][0][0][data], g_iPreStrafe[1][0][0][data], g_iPreStrafe[2][0][0][data]);
...
public void db_updateRecordPro(int client)
{
	...
	Format(szQuery, sizeof(szQuery), sql_updateRecordPro, szName, g_fFinalTime[client], g_iPreStrafe[0][0][0][client], g_iPreStrafe[1][0][0][client], g_iPreStrafe[2][0][0][client], g_szSteamID[client], g_szMapName, 0);
	SQL_TQuery(g_hDb, SQL_UpdateRecordProCallback, szQuery, pack, DBPrio_Low);
}
...
public void SQL_UpdateRecordProCallback2(Handle owner, Handle hndl, const char[] error, any data)
{
	...
	if(g_MapRank[client] != g_OldMapRank[client]){
		...
		db_UpdateTrack(g_szMapName, szEscName, g_OldMapRank[client], g_MapRank[client], 0, false);
	}
...
public void db_viewRecordCheckpointSpeedsInMap()
{
	...
	SQL_TQuery(g_hDb, sql_selectRecordCheckpointSpeedsCallback, szQuery, 1, DBPrio_Low);
}

public void sql_selectRecordCheckpointSpeedsCallback(Handle owner, Handle hndl, const char[] error, any data)
{
	if (hndl == null)
	{
		LogError("[SurfTimer] SQL Error (sql_selectRecordCheckpointSpeedsCallback): %s", error);
		if (!g_bServerDataLoaded)
			db_CalcAvgRunTime();
		return;
	}
...
public void db_LoadCPTypesSpeeds()
{
	...
	for(int i = 0; i < 6; i++){
		...
		if(i == 0){
			Format(szQuery, sizeof(szQuery), sql_selectCheckpointsSpeedsType, g_szMapName, g_szMapName, 10);
			SQL_TQuery(g_hDb, sql_selectCheckpointsSpeedsTypeCallback, szQuery, i, DBPrio_Low);
		}
		else if(i == 1){
			Format(szQuery, sizeof(szQuery), sql_selectCheckpointsSpeedsType, g_szMapName, g_szMapName, g_G1Top);
			SQL_TQuery(g_hDb, sql_selectCheckpointsSpeedsTypeCallback, szQuery, i, DBPrio_Low);
		}
		else if(i == 2){
			Format(szQuery, sizeof(szQuery), sql_selectCheckpointsSpeedsType, g_szMapName, g_szMapName, g_G2Top);
			SQL_TQuery(g_hDb, sql_selectCheckpointsSpeedsTypeCallback, szQuery, i, DBPrio_Low);
		}
		else if(i == 3){
			Format(szQuery, sizeof(szQuery), sql_selectCheckpointsSpeedsType, g_szMapName, g_szMapName, g_G3Top);
			SQL_TQuery(g_hDb, sql_selectCheckpointsSpeedsTypeCallback, szQuery, i, DBPrio_Low);
		}
		else if(i == 4){
			Format(szQuery, sizeof(szQuery), sql_selectCheckpointsSpeedsType, g_szMapName, g_szMapName, g_G4Top);
			SQL_TQuery(g_hDb, sql_selectCheckpointsSpeedsTypeCallback, szQuery, i, DBPrio_Low);
		}
		else if(i == 5){
			Format(szQuery, sizeof(szQuery), sql_selectCheckpointsSpeedsType, g_szMapName, g_szMapName, g_G5Top);
			SQL_TQuery(g_hDb, sql_selectCheckpointsSpeedsTypeCallback, szQuery, i, DBPrio_Low);
		}
...
public void SQL_selectCheckpointsCallback(Handle owner, Handle hndl, const char[] error, any client)
{
	...
	{	
		...
		while (SQL_FetchRow(hndl))
		{
			...
			cp = SQL_FetchInt(hndl, 1);
			g_fCheckpointTimesRecord[0][client][cp-1] = SQL_FetchFloat(hndl, 2);
		}

		db_CheckpointSpeeds(client);
	}

}

public void db_CheckpointSpeeds(int client)
{
	char szQuery[1024];
	Format(szQuery, sizeof(szQuery), sql_selectCheckpointsSpeeds, g_szMapName, g_szSteamID[client]);
	SQL_TQuery(g_hDb, SQL_CheckpointSpeedsCallback, szQuery, client, DBPrio_Low);
}

public void SQL_CheckpointSpeedsCallback(Handle owner, Handle hndl, const char[] error, any client)
{
	if (hndl == null)
	{
	...
	if (SQL_HasResultSet(hndl))
	{
		...
		while (SQL_FetchRow(hndl))
		{
			...
			g_bCheckpointSpeedsFound[client] = true;

			g_fCheckpointSpeedsRecord[client][cp-1] = SQL_FetchFloat(hndl, 1);
		}
	}
...
public void db_viewCheckpointSpeedsinZoneGroup(int client, char szSteamID[32], char szMapName[128], int zonegroup)
{
	char szQuery[1024];

	Format(szQuery, sizeof(szQuery), sql_selectCheckpointSpeedsinZoneGroup, szMapName, szSteamID, zonegroup);
	SQL_TQuery(g_hDb, db_viewCheckpointSpeedsinZoneGroupCallback, szQuery, client, DBPrio_Low);
}

public void db_viewCheckpointSpeedsinZoneGroupCallback(Handle owner, Handle hndl, const char[] error, any client)
{
	if (hndl == null)
	{
		LogError("[SurfTimer] SQL Error (db_viewCheckpointSpeedsinZoneGroupCallback): %s", error);
		return;
	}
	...
	{
		g_bCheckpointSpeedsFound[client] = false;
	}
}
...
public void db_viewCheckpointsinZoneGroupCallback(Handle owner, Handle hndl, const char[] error, any pack)
{
	...
	if (SQL_HasResultSet(hndl))
	{
		...
		db_viewCheckpointSpeedsinZoneGroup(client, g_szSteamID[client], g_szMapName, zonegrp);
	}
	else
	{
...
public void db_UpdateCheckpoints(int client, char szSteamID[32], int zGroup)
{
	...
	if (g_bCheckpointsFound[zGroup][client])
	{
		for(int i = 0; i < cp_count; i++){
			Format(szQuery, sizeof(szQuery), sql_updateCheckpoints, g_fCheckpointTimesNew[zGroup][client][i], g_fCheckpointSpeedsNew[client][i], szSteamID, g_szMapName, i+1, zGroup);
	...
	{
		for(int i = 0; i < cp_count; i++){
			Format(szQuery, sizeof(szQuery), sql_insertCheckpoints, szSteamID, g_szMapName, i+1, g_fCheckpointTimesNew[zGroup][client][i], g_fCheckpointSpeedsNew[client][i], zGroup);
			tAction.AddQuery(szQuery);
		}
	}
...
{
	PrintToServer("[SurfTimer] Checkpoints Update Successfull!")
	ResetPack(pack);
	int client = ReadPackCell(pack);
	int zonegrp = ReadPackCell(pack);
	CloseHandle(pack);
...
public void db_UpdateCCP(int client, int zGroup)
{
	Handle pack = CreateDataPack();
	WritePackCell(pack, client);
	WritePackCell(pack, zGroup);
	...
	SQL_ExecuteTransaction(g_hDb, tAction, db_UpdateCCPOnSuccess, db_UpdateCCPOnFailure, pack, DBPrio_Low);
}

public void db_UpdateCCPOnSuccess(Handle db, any pack, int numQueries, Handle[] results, any[] queryData)
{
	PrintToServer("[SurfTimer] CCP Update Successfull!")
	ResetPack(pack);
	int client = ReadPackCell(pack);
	int zonegrp = ReadPackCell(pack);
	CloseHandle(pack);
...
public void db_viewPRinfo(int client,  char szSteamID[32], char szMapName[128])
{
	...
	char szQuery[1024];
	Format(szQuery, sizeof(szQuery), sql_selectPR, szSteamID, szMapName, 0);
...
{	
	...
	if(SQL_HasResultSet(hndl) && SQL_FetchRow(hndl)){
		...
		Format(szQuery, sizeof(szQuery), "SELECT runtimepro FROM ck_playertimes WHERE steamid = '%s' AND style = 0 AND mapname = '%s';", szSteamID, szMapName);
...
public void db_UpdatePRinfo_WithRuntime(int client, char szSteamID[32], int zGroup, float runtime)
{
	...
	char szQuery[2048];
	Format(szQuery, sizeof(szQuery), sql_updatePrinfo_withruntime, g_fTimeinZone[client][zGroup], g_fCompletes[client][zGroup], g_fAttempts[client][zGroup], g_fstComplete[client][zGroup], runtime, szSteamID, g_szMapName, zGroup);
	SQL_TQuery(g_hDb, SQL_CheckCallback, szQuery, _, DBPrio_Low);
}
...
public void db_viewMapRankBonusCallback(Handle owner, Handle hndl, const char[] error, any data)
{
	...
	if (g_MapRankBonus[zgroup][client] <= 10 && g_MapRankBonus[zgroup][client] > 1)
		g_bBonusTop10Time[client] = true;
	else
		g_bBonusTop10Time[client] = false;
...
public void db_insertBonus(int client, char szSteamId[32], char szUName[128], float FinalTime, int zoneGrp)
{
	...
	Format(szQuery, sizeof(szQuery), sql_insertBonus, szSteamId, szName, g_szMapName, FinalTime, zoneGrp, g_iPreStrafeBonus[0][zoneGrp][0][client], g_iPreStrafeBonus[1][zoneGrp][0][client], g_iPreStrafeBonus[2][zoneGrp][0][client]);
...
public void db_updateBonus(int client, char szSteamId[32], char szUName[128], float FinalTime, int zoneGrp)
{
	...
	SQL_EscapeString(g_hDb, szUName, szName, MAX_NAME_LENGTH * 2 + 1);
	Format(szQuery, sizeof(szQuery), sql_updateBonus,FinalTime, szName, g_iPreStrafeBonus[0][zoneGrp][0][client], g_iPreStrafeBonus[1][zoneGrp][0][client], g_iPreStrafeBonus[2][zoneGrp][0][client], szSteamId, g_szMapName, zoneGrp);
...
{	
	char szQuery[2048];
	Format(szQuery, sizeof(szQuery), sql_RecentlyLost, g_szSteamID[client]);
...
{	
	char szQuery[2048];
	Format(szQuery, sizeof(szQuery), sql_Tracking, g_szSteamID[client]);
...
{	
	char szQuery[1024];
	Format(szQuery, sizeof(szQuery), sql_InsertTrack_All, szSteamID);
	SQL_TQuery(g_hDb, SQL_CheckCallback, szQuery, _, DBPrio_Low);
...
{
	char szQuery[1024];
	Format(szQuery, sizeof(szQuery), sql_GetPlayerWipedRanks, szSteamID);
	SQL_TQuery(g_hDb, sql_GetPlayerWipedRanksCallback, szQuery, _, DBPrio_Low);
}
...
public void sql_GetPlayerWipedRanksCallback(Handle owner, Handle hndl, const char[] error, any data)
{
	...
	{	
		...
		while (SQL_FetchRow(hndl))
		{
			...
			char szQuery[1024];
			Format(szQuery, sizeof(szQuery), sql_UpdatePlayersRanks, szMapName, zonegroup, previous_rank);
			SQL_TQuery(g_hDb, SQL_CheckCallback, szQuery, _, DBPrio_Low);
...
public void db_InsertTrack(char szSteamID[32], char szName[MAX_NAME_LENGTH * 2 + 1], char szMapName[128], int zonegroup, int previous_rank, int new_rank)
{	
	char szQuery[1024];
	Format(szQuery, sizeof(szQuery), sql_InsertTrack, szSteamID, szName, szMapName, zonegroup, previous_rank, new_rank);
	SQL_TQuery(g_hDb, SQL_CheckCallback, szQuery, _, DBPrio_Low);

	//db_UpdateTrack(szMapName, szName, new_rank, zonegroup, false);
}

public void db_UpdateTrack(char szMapName[128],char szName[MAX_NAME_LENGTH * 2 + 1], int prev_rank, int new_rank, int zonegroup, bool deleting)
{

	Handle pack = CreateDataPack();
	WritePackString(pack, szName);
	WritePackCell(pack, new_rank);
...
public void sql_GetPlayersToUpdateCallback(Handle owner, Handle hndl, const char[] error, any data)
{
	if (hndl == null)
	{
		LogError("[SurfTimer] SQL Error (sql_GetPlayersToUpdateCallback): %s", error);
		CloseHandle(data);
		return;
	}
	...
	{	
		...
		ResetPack(data);
		ReadPackString(data, szName, MAX_NAME_LENGTH * 2 + 1);
		...
		PlayersCheck = new ArrayList(MAX_NAME_LENGTH * 2 + 1);
		...
		{
			SQL_FetchString(hndl, 0, szSteamID, 32);
			SQL_FetchString(hndl, 1, szName, MAX_NAME_LENGTH * 2 + 1);
			...
			if(deleting){
				if(new_rank >= player_rank){
					char szQuery[1024];
					Format(szQuery, sizeof(szQuery), sql_InsertTrack, szSteamID, szName, szMapName, zonegroup,  previous_rank, new_rank - 1);
					SQL_TQuery(g_hDb, SQL_CheckCallback, szQuery, _, DBPrio_Low);
				}
			}
			else{
...
public void db_Cleanup()
{
	...
	// times
	SQL_TQuery(g_hDb, SQL_CheckCallback, "DELETE FROM ck_playertimes where runtimepro = -1.0", DBPrio_Low);

	// fluffys pointless players
	SQL_TQuery(g_hDb, SQL_CheckCallback, "DELETE FROM ck_playerrank WHERE `points` <= 0", DBPrio_Low);
}
...
public void SQL_CheckCallback(Handle owner, Handle hndl, const char[] error, any data)
{
	if (hndl == null)
	{
		LogError("[SurfTimer] SQL Error (SQL_CheckCallback): %s | Query : %s", error, data);
		return;
	}
}
...
public void SQL_CheckCallback2(Handle owner, Handle hndl, const char[] error, any data)
{
	if (hndl == null)
	{
		LogError("[SurfTimer] SQL Error (SQL_CheckCallback2): %s | Query : %s", error, data);
		return;
	}
...
public void SQL_CheckCallback3(Handle owner, Handle hndl, const char[] error, any data)
{
	if (hndl == null)
	{
		LogError("[SurfTimer] SQL Error (SQL_CheckCallback3): %s | Query : %s", error, data);
		return;
	}
...
public void SQL_CheckCallback4(Handle owner, Handle hndl, const char[] error, any data)
{
	if (hndl == null)
	{
		LogError("[SurfTimer] SQL Error (SQL_CheckCallback4): %s | Query : %s", error, data);
		return;
	}
	char steamid[128];
...
public void db_viewPlayerOptionsCallback(Handle owner, Handle hndl, const char[] error, any client)
{
	...
	if (SQL_HasResultSet(hndl) && SQL_FetchRow(hndl))
	{
		...
		g_bHideChat[client] = view_as<bool>(SQL_FetchInt(hndl, 2));
		g_bViewModel[client] = view_as<bool>(SQL_FetchInt(hndl, 3));
		g_bAutoBhopClient[client] = view_as<bool>(SQL_FetchInt(hndl, 4));
		g_bCheckpointsEnabled[client] = view_as<bool>(SQL_FetchInt(hndl, 5));
		g_SpeedGradient[client] = SQL_FetchInt(hndl, 6);
		g_SpeedMode[client] = SQL_FetchInt(hndl, 7);
		g_bCenterSpeedDisplay[client] = view_as<bool>(SQL_FetchInt(hndl, 8));
		g_bCentreHud[client] = view_as<bool>(SQL_FetchInt(hndl, 9));
		g_iTeleSide[client] = SQL_FetchInt(hndl, 10);
		...
		g_iPrespeedText[client] = view_as<bool>(SQL_FetchInt(hndl, 23));
		g_PreSpeedMode[client] = SQL_FetchInt(hndl, 24);
		...
		g_iCSDUpdateRate[client] = SQL_FetchInt(hndl, 29);
		g_fCSD_POS_X[client] = SQL_FetchFloat(hndl, 30);
		g_fCSD_POS_Y[client] = SQL_FetchFloat(hndl, 31);
		g_iCSD_R[client] = SQL_FetchInt(hndl, 32);
		g_iCSD_G[client] = SQL_FetchInt(hndl, 33);
		g_iCSD_B[client] = SQL_FetchInt(hndl, 34);
		g_iCustomCheckpointCompareType[client] = SQL_FetchInt(hndl, 35);
		g_iDefaultStyle[client] = SQL_FetchInt(hndl, 36);
...
public void db_updatePlayerOptions(int client)
{
	...
	{
		Format(szQuery, sizeof(szQuery), sql_updatePlayerOptions, BooltoInt(g_bTimerEnabled[client]), BooltoInt(g_bHide[client]), BooltoInt(g_bHideChat[client]), BooltoInt(g_bViewModel[client]), BooltoInt(g_bAutoBhopClient[client]), BooltoInt(g_bCheckpointsEnabled[client]), g_SpeedGradient[client], g_SpeedMode[client], BooltoInt(g_bCenterSpeedDisplay[client]), BooltoInt(g_bCentreHud[client]), g_iTeleSide[client], g_iCentreHudModule[client][0], g_iCentreHudModule[client][1], g_iCentreHudModule[client][2], g_iCentreHudModule[client][3], g_iCentreHudModule[client][4], g_iCentreHudModule[client][5], BooltoInt(g_bSideHud[client]), g_iSideHudModule[client][0], g_iSideHudModule[client][1], g_iSideHudModule[client][2], g_iSideHudModule[client][3], g_iSideHudModule[client][4], BooltoInt(g_iPrespeedText[client]), g_PreSpeedMode[client], BooltoInt(g_iCpMessages[client]), BooltoInt(g_iWrcpMessages[client]), BooltoInt(g_bAllowHints[client]), BooltoInt(g_bTimeleftDisplay[client]), g_iCSDUpdateRate[client], g_fCSD_POS_X[client], g_fCSD_POS_Y[client], g_iCSD_R[client], g_iCSD_G[client], g_iCSD_B[client], g_iCustomCheckpointCompareType[client], g_iDefaultStyle[client], g_szSteamID[client]);
		SQL_TQuery(g_hDb, SQL_CheckCallback, szQuery, _, DBPrio_Low);
	}
}
...
{
	char szQuery[512];
	Format(szQuery, sizeof(szQuery), "SELECT COUNT(DISTINCT a.mapname,zonegroup) as count FROM ck_zones a RIGHT JOIN ck_maptier b ON a.mapname = b.mapname WHERE a.zonegroup > 0;");
	SQL_TQuery(g_hDb, sql_selectTotalBonusCountCallback, szQuery, _, DBPrio_Low);
}
...
public void db_selectTotalStageCount()
{
	...
	SQL_TQuery(g_hDb, sql_selectTotalStageCountCallback, szQuery, _, DBPrio_Low);
}
...
public void sql_viewWrcpMapCallback(Handle owner, Handle hndl, const char[] error, any pack)
{
	...
	if (SQL_HasResultSet(hndl) && SQL_FetchRow(hndl))
	{
		...
		if (pack != INVALID_HANDLE)
		{
			...
			/*// Find out how many times are are faster than the players time
			char szQuery[512];
			Format(szQuery, sizeof(szQuery), "", g_szMapName, g_CurrentStage[data], stagetime);
			SQL_TQuery(g_hDb, SQL_UpdateRecordProCallback2, szQuery, client, DBPrio_Low);*/
		}
	}
}
...
public void db_updatePersonalStyleRecord(int client, int style)
{
	...
	char szQuery[1024];
	Format(szQuery, sizeof(szQuery), "UPDATE `ck_playertimes` SET `name` = '%s', runtimepro = '%f', `velStartXY` = %i, `velStartXYZ` = %i, `velStartZ` = %i WHERE `steamid` = '%s' AND `mapname` = '%s' AND `style` = %i;", szName, g_fFinalTime[client], g_iPreStrafe[0][0][style], g_iPreStrafe[1][0][style], g_iPreStrafe[2][0][style], g_szSteamID[client], g_szMapName, style);
	SQL_TQuery(g_hDb, SQL_UpdatePersonalStyleRecordCallback, szQuery, pack, DBPrio_Low);
}
...
{
	char szQuery[1024];
	Format(szQuery, sizeof(szQuery), sql_selectTopSurfers, mapname, style, style);
	Handle pack = CreateDataPack();
	WritePackCell(pack, client);
	WritePackString(pack, mapname);
	WritePackCell(pack, style);
...
public void db_insertBonusStyle(int client, char szSteamId[32], char szUName[128], float FinalTime, int zoneGrp, int style)
{
	...
	WritePackCell(pack, zoneGrp);
	WritePackCell(pack, style);
	Format(szQuery, sizeof(szQuery), "INSERT INTO ck_bonus (steamid, name, mapname, runtime, zonegroup, style, velStartXY, velStartXYZ, velStartZ) VALUES ('%s', '%s', '%s', '%f', '%i', '%i', '%i', '%i', '%i')", szSteamId, szName, g_szMapName, FinalTime, zoneGrp, style, g_iPreStrafeBonus[0][zoneGrp][style][client], g_iPreStrafeBonus[1][zoneGrp][style][client], g_iPreStrafeBonus[2][zoneGrp][style][client]);
...
public void db_viewMapRankBonusStyleCallback(Handle owner, Handle hndl, const char[] error, any data)
{
	...
	if (g_StyleMapRankBonus[style][zgroup][client] <= 10 && g_StyleMapRankBonus[style][zgroup][client] > 1)
		g_bBonusTop10Time[client] = true;
	else
		g_bBonusTop10Time[client] = false;
...
public void db_SelectTotalMapCompletesCallback(Handle owner, Handle hndl, const char[] error, any pack)
{
	...
	if (SQL_HasResultSet(hndl) && SQL_FetchRow(hndl))
	{
		...
		Format(szQuery, sizeof(szQuery), "SELECT name,mapname FROM ck_playertimes WHERE runtimepro <= (SELECT runtimepro FROM ck_playertimes WHERE steamid = '%s' AND mapname = '%s' AND runtimepro > -1.0 AND style = 0) AND mapname = '%s' AND style = 0 AND runtimepro > -1.0 ORDER BY runtimepro;", szSteamId, mapname, mapname);
...
public void db_selectMapRankUnknownWithMap(int client, char szMapName[128], int rank)
{
    ...
    Format(szQuery, sizeof(szQuery), sql_selectMapRankUnknownWithMap, szMapName, rank);
    SQL_TQuery(g_hDb, db_selectMapRankUnknownCallback, szQuery, pack, DBPrio_Low);
}
...
public void db_selectPlayerRank(int client, int rank, char szSteamId[32])
{
	...
	if (StrContains(szSteamId, "none", false)!= -1 && rank > 0) // Select Rank Number
	{
		...
		rank -= 1;
		Format(szQuery, sizeof(szQuery), "SELECT `name`, `points` FROM `ck_playerrank` WHERE `style` = 0 ORDER BY `points` DESC LIMIT %i, 1;", rank);
	}
	else if (rank == 0) // Self Rank Cmd
	{
		g_rankArg[client] = -1;
		Format(szQuery, sizeof(szQuery), "SELECT `name`, `points` FROM `ck_playerrank` WHERE `steamid` = '%s' AND `style` = 0;", szSteamId);
	}
...
public void db_checkCustomPlayerTitle(int client, char[] arg)
{
	...
	char szQuery[512];
	Format(szQuery, sizeof(szQuery), "SELECT `steamid` FROM `ck_vipadmins` WHERE `steamid` = '%s';", g_szSteamID[client]);
...
public void db_selectPRinfoUnknown(int client, int rank, int zonegroup, char szSteamID[32])
{
	...
	Format(szQuery, sizeof(szQuery), "SELECT steamid, name, mapname, runtime, PRtimeinzone, PRcomplete, PRattempts, PRstcomplete FROM ck_prinfo WHERE mapname LIKE '%c%s%c' AND zonegroup = '%i' AND steamid = '%s';", PERCENT, g_szMapName, PERCENT, zonegroup, szSteamID);
...
public void db_selectPRinfoUnknownWithMap(int client, int rank, char szMapName[128], int zonegroup, char szSteamID[32])
{
	...
	Format(szQuery, sizeof(szQuery), "SELECT steamid, name, mapname, runtime, PRtimeinzone, PRcomplete, PRattempts, PRstcomplete FROM ck_prinfo WHERE mapname = '%s' AND zonegroup = '%i' AND steamid = '%s';", szMapName, zonegroup, szSteamID);
	SQL_TQuery(g_hDb, db_selectPRinfoUnknownCallback, szQuery, pack, DBPrio_Low);
}
...
public void db_viewPRinfoMapRank(int client, char szSteamID[32], char szMapName[128])
{
	...
	Format(szQuery, sizeof(szQuery), "SELECT COUNT(*),steamid FROM ck_playertimes WHERE runtimepro <= (SELECT runtimepro FROM ck_playertimes WHERE steamid = '%s' AND mapname LIKE '%c%s%c' AND style = 0 AND runtimepro > -1.0) AND mapname LIKE '%c%s%c' AND style = 0 AND runtimepro > -1.0;", szSteamID, PERCENT, szMapName, PERCENT, PERCENT, szMapName, PERCENT);
...
public void db_viewPRinfoMapRankBonus(int client, char szSteamID[32], char szMapName[128], int zonegroup)
{
	...
	Format(szQuery, sizeof(szQuery), "SELECT COUNT(*), steamid FROM ck_bonus WHERE runtime <= (SELECT runtime FROM ck_bonus WHERE steamid = '%s' AND mapname LIKE '%c%s%c' AND runtime > -1.0 AND zonegroup = %i AND style = 0) AND mapname = '%s' AND zonegroup = %i AND style = 0;", szSteamID, PERCENT, szMapName, PERCENT, zonegroup, szMapName, zonegroup);
...
public void db_GetRankSteamID(int client, char szMapName[128], int rank, int zonegroup)
{
	...
	if(zonegroup == 0){
		Format(szQuery, sizeof(szQuery), "SELECT steamid FROM ck_playertimes WHERE mapname = '%s' AND style = 0 AND runtimepro > -1.0 ORDER BY runtimepro ASC LIMIT %i, 1;", szMapName, rank - 1);
		SQL_TQuery(g_hDb, SQL_GetRankSteamIDCallback, szQuery, pack, DBPrio_Low);
	}
	else{
		Format(szQuery, sizeof(szQuery), "SELECT steamid FROM ck_bonus WHERE mapname = '%s' AND style = 0 AND runtime > -1.0 AND zonegroup = '%i' ORDER BY runtime ASC LIMIT %i, 1;", szMapName, zonegroup, rank - 1);
		SQL_TQuery(g_hDb, SQL_GetRankSteamIDCallback, szQuery, pack, DBPrio_Low);
	}
...
public void db_viewCCP_GetPlayerPR(int client, char szSteamID[32], char szMapName[128], float map_time, int map_rank, float record_time, int total_map_completions){
	...
	char szQuery[2048];
	Format(szQuery, sizeof szQuery, "SELECT db1.steamid, db1.mapname, db1.cp, db1.stage_time, db1.stage_attempts, (SELECT count(name)+1 FROM ck_wrcps WHERE style = 0 AND mapname = db1.mapname AND stage = db1.cp AND stage_time > -1.0 AND runtimepro <= db1.stage_time) AS `rank`, (SELECT count(name) FROM ck_wrcps WHERE style = 0 AND mapname = db1.mapname AND stage = db1.cp AND runtimepro > -1.0) AS total FROM ck_ccp db1 WHERE db1.mapname = '%s' AND db1.steamid = '%s' AND db1.stage_time > -1.0 ORDER BY cp ASC;", szMapName, szSteamID);
...
public void sql_getMapAttributesCallback(Handle owner, Handle hndl, const char[] error, any data)
{
	...
	if (SQL_HasResultSet(hndl) && SQL_FetchRow(hndl))
	{
		...
		LogToFileEx(g_szLogFile, "[SurfTimer] Map Attributes Updated!");
	}
}
...
//SOUNDS
/////
public void db_LoadClientSounds(int client)
{
	char szQuery[2048];
	Format(szQuery, sizeof szQuery, sql_selectSoundOptions, g_szSteamID[client]);
	SQL_TQuery(g_hDb, LoadClientSoundsCallback, szQuery, client, DBPrio_Low);
}

public void LoadClientSoundsCallback(Handle owner, Handle hndl, const char[] error, any client)
{
	if (hndl == null)
	{
		LogError("[SurfTimer] SQL Error (db_viewPlayerOptionsCallback): %s", error);
	...
	if (SQL_HasResultSet(hndl) && SQL_FetchRow(hndl)) {
		...
		{
			g_iClientSounds[client][i] = SQL_FetchInt(hndl, i + 1);
		}
	}
	else {
		char szQuery[2048];
		Format(szQuery, sizeof szQuery, sql_insertSoundOptions, g_szSteamID[client]);
		SQL_TQuery(g_hDb, SQL_CheckCallback, szQuery, _, DBPrio_Low);
	}
...
public void db_updatePlayerSound(int client)
{
	if (!IsValidClient(client)) {
		return;
	}
	...
	char szQuery[2048];
	Format(szQuery, sizeof(szQuery), sql_updateSoundOptions, BooltoInt(g_bEnableQuakeSounds[client]), g_iClientSounds[client][0], g_iClientSounds[client][1], g_iClientSounds[client][2], g_iClientSounds[client][3], g_iClientSounds[client][4], g_iClientSounds[client][5], g_iClientSounds[client][6], g_iClientSounds[client][7], g_iClientSounds[client][8], g_iClientSounds[client][9], g_szSteamID[client]);
	SQL_TQuery(g_hDb, SQL_CheckCallback, szQuery, _, DBPrio_Low);
}