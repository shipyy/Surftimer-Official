...
{	
	...
	switch(option){
		case 0: Format(szQuery, 512, sql_MapRecords, g_szSteamID[client], g_ProfileStyleSelect[client]);
		case 1:	Format(szQuery, 512, sql_StageRecords, g_szSteamID[client], g_ProfileStyleSelect[client]);
		case 2: Format(szQuery, 512, sql_BonusRecords, g_szSteamID[client], g_ProfileStyleSelect[client]);
	}
...
public void db_viewPlayerOptionsCallback(Handle owner, Handle hndl, const char[] error, any client)
{
	...
	if (SQL_HasResultSet(hndl) && SQL_FetchRow(hndl))
	{
		...
		g_iCSDUpdateRate[client] = SQL_FetchInt(hndl, 30);
		g_fCSD_POS_X[client] = SQL_FetchFloat(hndl, 31);
		g_fCSD_POS_Y[client] = SQL_FetchFloat(hndl, 32);
		g_iCSD_R[client] = SQL_FetchInt(hndl, 33);
		g_iCSD_G[client] = SQL_FetchInt(hndl, 34);
		g_iCSD_B[client] = SQL_FetchInt(hndl, 35);
		g_iCustomCheckpointCompareType[client] = SQL_FetchInt(hndl, 36);
...
public void sql_selectStyleRecordCallback(Handle owner, Handle hndl, const char[] error, any stylepack)
{
	...
	Format(szQuery, sizeof(szQuery), "INSERT INTO ck_playertimes (steamid, mapname, name, runtimepro, style, velStartXY, velStartXYZ, velStartZ) VALUES ('%s', '%s', '%s', '%f', %i, %i, %i, %i)", g_szSteamID[data], g_szMapName, szName, g_fFinalTime[data], style, g_iPreStrafe[0][0][style], g_iPreStrafe[1][0][style], g_iPreStrafe[2][0][style]);
