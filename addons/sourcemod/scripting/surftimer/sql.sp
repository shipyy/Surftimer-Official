...
public void db_viewRecordCheckpointInMap()
{
	...
	char szQuery[1028];
	Format(szQuery, 1028, sql_selectRecordCheckpoints, g_szMapName, g_szMapName);
...
public void db_viewRecordCheckpointSpeedsInMap()
{
	...
	// "SELECT c.zonegroup, c.cp1, c.cp2, c.cp3, c.cp4, c.cp5, c.cp6, c.cp7, c.cp8, c.cp9, c.cp10, c.cp11, c.cp12, c.cp13, c.cp14, c.cp15, c.cp16, c.cp17, c.cp18, c.cp19, c.cp20, c.cp21, c.cp22, c.cp23, c.cp24, c.cp25, c.cp26, c.cp27, c.cp28, c.cp29, c.cp30, c.cp31, c.cp32, c.cp33, c.cp34, c.cp35 FROM ck_checkpoints c WHERE steamid = '%s' AND mapname='%s' UNION SELECT a.zonegroup, b.cp1, b.cp2, b.cp3, b.cp4, b.cp5, b.cp6, b.cp7, b.cp8, b.cp9, b.cp10, b.cp11, b.cp12, b.cp13, b.cp14, b.cp15, b.cp16, b.cp17, b.cp18, b.cp19, b.cp20, b.cp21, b.cp22, b.cp23, b.cp24, b.cp25, b.cp26, b.cp27, b.cp28, b.cp29, b.cp30, b.cp31, b.cp32, b.cp33, b.cp34, b.cp35 FROM ck_bonus a LEFT JOIN ck_checkpoints b ON a.steamid = b.steamid AND a.zonegroup = b.zonegroup WHERE a.mapname = '%s' GROUP BY a.zonegroup";
	char szQuery[1028];
	Format(szQuery, 1028, sql_selectRecordCheckpointSpeeds, g_szMapName, g_szMapName);
...
{	
	...
	Format(szQuery, 2048, sql_updatePrinfo, g_fTimeinZone[client][zGroup], g_fCompletes[client][zGroup], g_fAttempts[client][zGroup], g_fstComplete[client][zGroup], szSteamID, g_szMapName, zGroup);
	SQL_TQuery(g_hDb, SQL_CheckCallback, szQuery, DBPrio_Low);
}
...
public void db_selectPRinfo(int client, int rank, const char szMapName[128], int zonegroup)
{
	Handle pack = CreateDataPack();
	WritePackCell(pack, client);
	...
	char szQuery[2048];
	Format(szQuery, sizeof(szQuery), "SELECT steamid, name, runtime, PRtimeinzone, PRcomplete, PRattempts, PRstcomplete FROM ck_prinfo WHERE steamid = '%s' AND mapname LIKE '%c%s%c' AND zonegroup = '%i';", g_szSteamID[client], PERCENT, szMapName, PERCENT, zonegroup);
	//PrintToConsole(client, "QUERY %s", szQuery);
	SQL_TQuery(g_hDb, SQL_SelectPRinfoCallback, szQuery, pack, DBPrio_Low);
}

public void SQL_SelectPRinfoCallback(Handle owner, Handle hndl, const char[] error, any pack)
{
	if (hndl == null)
	{
		LogError("[surftimer] SQL Error (SQL_SelectPRTimeCallback): %s", error);
		return;
	}
	...
	{	
		...
		SQL_FetchString(hndl, 0, szSteamId, 32);
		SQL_FetchString(hndl, 1, szPlayerName, 32);

		runtime = SQL_FetchFloat(hndl, 2);
		PRTimeinZone = SQL_FetchFloat(hndl, 3);
		PRCompletes = SQL_FetchFloat(hndl, 4);
		PRAttempts = SQL_FetchFloat(hndl, 5);
		PRstcomplete = SQL_FetchFloat(hndl, 6);
...
public void db_selectPRinfoUnknown(int client, int rank, char szMapName[128], int zonegroup)
{
	char szQuery[1024];
	Handle pack = CreateDataPack();
	WritePackCell(pack, client);
	...
	Format(szQuery, 1024, "SELECT steamid, name, mapname, runtime, PRtimeinzone, PRcomplete, PRattempts, PRstcomplete FROM ck_prinfo WHERE mapname LIKE '%c%s%c' AND zonegroup = '%i' ORDER BY runtime ASC LIMIT %i, 1;", PERCENT, szMapName, PERCENT, zonegroup, rank);
	//PrintToConsole(client, "QUERY %s", szQuery);
...
public void db_selectPRinfoUnknownWithMap(int client, int rank, char szMapName[128], int zonegroup)
{
	char szQuery[1024];
	Handle pack = CreateDataPack();
	WritePackCell(pack, client);
	...
	Format(szQuery, 1024, "SELECT steamid, name, mapname, runtime, PRtimeinzone, PRcomplete, PRattempts, PRstcomplete FROM ck_prinfo WHERE mapname = '%s' AND zonegroup = '%i' ORDER BY runtime ASC LIMIT %i, 1;", szMapName, zonegroup, rank);
	//PrintToConsole(client, "QUERY %s", szQuery);
	SQL_TQuery(g_hDb, db_selectPRinfoUnknownCallback, szQuery, pack, DBPrio_Low);
}
...
public void db_viewPRinfoMapRank(int client, char szSteamID[32], char szMapName[128])
{
	...
	Format(szQuery, 1024, "SELECT COUNT(*) FROM ck_playertimes WHERE runtimepro <= (SELECT runtimepro FROM ck_playertimes WHERE steamid = '%s' AND mapname LIKE '%c%s%c' AND style = 0 AND runtimepro > -1.0) AND mapname LIKE '%c%s%c' AND style = 0 AND runtimepro > -1.0;", szSteamID, PERCENT, szMapName, PERCENT, PERCENT, szMapName, PERCENT);
...
public void db_viewPRinfoMapRankBonus(int client, char szSteamID[32], char szMapName[128], int zonegroup)
{
	...
	Format(szQuery, 1024, "SELECT COUNT(*) FROM ck_bonus WHERE runtime <= (SELECT runtime FROM ck_bonus WHERE steamid = '%s' AND mapname LIKE '%c%s%c' AND runtime > 0.0 AND zonegroup = %i AND style = 0) AND mapname = '%s' AND zonegroup = %i AND style = 0;", szSteamID, PERCENT, szMapName, PERCENT, zonegroup, szMapName, zonegroup);
...
public void db_viewPRinfoMapRankBonusCallback(Handle owner, Handle hndl, const char[] error, any pack)
{
	...
	{	
		...
		if( g_iPRinfoMapRankBonus[client] == 0)
			db_selectPRinfo(client, g_iPRinfoMapRankBonus[client], szMapName, bonus_number);
		else
			db_selectPRinfoUnknownWithMap(client, g_iPRinfoMapRankBonus[client], szMapName, bonus_number);
...
public void db_GetTotalMapCompletions(int client, char szSteamID[32], char szMapName[128], float map_time, int map_rank, char szMapTimeFormatted[32]){
	...
	Format(szQuery, 1024, "SELECT count(runtimepro)+1 FROM ck_playertimes WHERE mapname = '%s' AND style = '0';", szMapName);
...
public void db_viewCCP_GetMapStageTimes_Record(int client, char szSteamID[32], char Record_SteamID[32], char szMapName[128], float map_time, int map_rank,int total_map_completions, float record_time , char szMapTimeFormatted[32], char szMapTimeDiffFormatted[32])
{
	...
	Format(szQuery, 2048, "SELECT cp1, cp2, cp3, cp4, cp5, cp6, cp7, cp8, cp9, cp10, cp11, cp12, cp13, cp14, cp15, cp16, cp17, cp18, cp19, cp20, cp21, cp22, cp23, cp24, cp25, cp26, cp27, cp28, cp29, cp30, cp31, cp32, cp33, cp34, cp35 FROM ck_checkpoints WHERE mapname = '%s' AND zonegroup = '0' AND steamid = '%s';", szMapName, Record_SteamID);
...
public void db_viewCCP_GetMapStageTimes_Player(int client, char szSteamID[32], char Record_SteamID[32], char szMapName[128], float map_time, int map_rank, float record_time, int total_map_completions, char szMapTimeFormatted[32], char szMapTimeDiffFormatted[32])
{
	...
	Format(szQuery, 2048, "SELECT cp1, cp2, cp3, cp4, cp5, cp6, cp7, cp8, cp9, cp10, cp11, cp12, cp13, cp14, cp15, cp16, cp17, cp18, cp19, cp20, cp21, cp22, cp23, cp24, cp25, cp26, cp27, cp28, cp29, cp30, cp31, cp32, cp33, cp34, cp35 FROM ck_checkpoints WHERE mapname = '%s' AND zonegroup = '0' AND steamid = '%s';", szMapName, szSteamID);
...
public void db_viewCCP_GetMapStageRank(int client, char szSteamID[32], char szMapName[128], int map_rank,int total_map_completions, char szMapTimeFormatted[32], char szMapTimeDiffFormatted[32], int stage, float stage_time, char szStageTimeFormatted[32])
{
	...
	char szQuery[2048];
	Format(szQuery, 2048, "SELECT count(runtimepro)+1 FROM ck_wrcps WHERE mapname = '%s' AND stage = '%i' AND style = '0' AND runtimepro < %f;", szMapName, stage+1, stage_time);
