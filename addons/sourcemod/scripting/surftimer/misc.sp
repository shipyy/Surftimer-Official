...
public void SetClientDefaults(int client)
{
	...
	Format(g_szLastSRDifferenceMinimalHUD[client], 64, "");
	Format(g_szLastPBDifferenceMinimalHUD[client], 64, "");
	...
	for (int x = 0; x < MAXZONEGROUPS; x++)
	{
		...
		for (int i = 0; i < CPLIMIT; i++)
		{
			...
			g_fCheckpointSpeedsNew[client][i] = 0.0;
...
public void MinimalHudAlive(int client){
	...
	if(g_bMinimalHUD[client]){
		...
		char s_Timer[64];
		char s_MapType[64];
		char s_MapWorldRecord[64];
		char s_MapPersonalRecord[64];
		int displayColor[3];
		...
		if ( (GetGameTime() - g_fLastDifferenceTime[client] <= 5.0) && g_bTimerRunning[client] && g_iCurrentStyle[client] == 0){
			...
			//else
				//Format(s_Timer, 64, "%s\n%s", s_Timer, g_szLastCustomCheckpointDifferenceMinimalHUD[client]);
		...
		if (g_iClientInZone[client][2] == 0)
		{
			...
			{
				Format(s_MapType, 64, "Linear Map");
			}
			else // map has stages
			{
				if (g_bPracticeMode[client])
				{
					...
					{
						Format(s_MapType, 64, "Stage: %i/%i", g_iPlayerPracLocationSnap[client][g_iPlayerPracLocationSnapIdClient[client]], (g_mapZonesTypeCount[g_iClientInZone[client][2]][3] + 1));
					}
					else
					{
						Format(s_MapType, 64, "Stage: %i/%i", g_Stage[g_iClientInZone[client][2]][client], (g_mapZonesTypeCount[g_iClientInZone[client][2]][3] + 1));
					}
				}
				else
				{
					Format(s_MapType, 64, "Stage: %i/%i", g_Stage[g_iClientInZone[client][2]][client], (g_mapZonesTypeCount[g_iClientInZone[client][2]][3] + 1)); // less \t's to make lines align
				}
			}
		}
		else
		{
			Format(s_MapType, 64, "Bonus %i", g_iClientInZone[client][2]);
		}
		...
		if (g_iClientInZone[client][2] == 0 && style == 0)
		{
			if (g_fRecordMapTime != 9999999.0)
			{
				// fluffys
				if (g_bPracticeMode[client])
					Format(s_MapWorldRecord, 64, "SR: %s", g_szRecordMapTime);
				else
					Format(s_MapWorldRecord, 64, "SR: %s", g_szRecordMapTime);
			}
			else
				Format(s_MapWorldRecord, 64, "SR: N/A");
		}
		else if (g_iClientInZone[client][2] == 0 && g_iCurrentStyle[client] != 0) // Styles
		{
			if (g_fRecordStyleMapTime[style] != 9999999.0)
			{
				// fluffys
				if (g_bPracticeMode[client])
					Format(s_MapWorldRecord, 64, "SR: %s", g_szRecordStyleMapTime[style]);
				else
					Format(s_MapWorldRecord, 64, "SR: %s", g_szRecordStyleMapTime[style]);
			}
			else
		...
		else
		{
			if (g_iCurrentStyle[client] == 0)
			{
				...
				{
					Format(s_MapWorldRecord, 64, "SR: %s", g_szBonusFastestTime[g_iClientInZone[client][2]]);
				}
				else
				{	
					Format(s_MapWorldRecord, 64, "SR: %s", g_szBonusFastestTime[g_iClientInZone[client][2]]);
			...
			else if (g_iCurrentStyle[client] != 0) // Styles
			{
				...
				{
					Format(s_MapWorldRecord, 64, "SR: %s", g_szStyleBonusFastestTime[style][g_iClientInZone[client][2]]);
				}
				else
				{
					Format(s_MapWorldRecord, 64, "SR: %s", g_szStyleBonusFastestTime[style][g_iClientInZone[client][2]]);
		...
		if (g_iClientInZone[client][2] == 0 && style == 0)
		{
			...
			{
				if (g_fPersonalRecord[client] > 0.0)
					Format(s_MapPersonalRecord, 64, "PB: %s", g_szPersonalRecord[client]);
				else
					Format(s_MapPersonalRecord, 64, "PB: N/A");
			}
			else
				Format(s_MapPersonalRecord, 64, "PB: N/A");
		}
		else if (g_iClientInZone[client][2] == 0 && g_iCurrentStyle[client] != 0) // Styles
		{
			if (g_fRecordStyleMapTime[style] != 9999999.0)
			{
				if (g_fPersonalStyleRecord[style][client] > 0.0)
					Format(s_MapPersonalRecord, 64, "PB: %s", g_szPersonalStyleRecord[style][client]);
				else
					Format(s_MapPersonalRecord, 64, "PB: N/A");
			}
			else
...
public void MinimalHudDead(int client){
	...
	if (SpecMode == 4 || SpecMode == 5)
	{
		...
		{	
			int style = g_iCurrentStyle[ObservedUser];
			char s_Timer[64];
			char s_MapType[64];
			char s_MapWorldRecord[64];
			char s_MapPersonalRecord[64];
			...
			if(!IsFakeClient(ObservedUser)){
				...
				if ( (GetGameTime() - g_fLastDifferenceTime[ObservedUser] <= 5.0) && g_iCurrentStyle[ObservedUser] == 0){
					...
					//else
						//Format(s_Timer, 64, "%s\n%s", s_Timer, g_szLastCustomCheckpointDifferenceMinimalHUD[ObservedUser]);
				...
				if (g_iClientInZone[ObservedUser][2] == 0)
				{
					...
					else // map has stages
					{
						if (g_bPracticeMode[ObservedUser])
						{
							if (g_bSaveLocTele[ObservedUser]) // Has the player teleported to saveloc?
							{
								Format(s_MapType, 64, "Stage: %i/%i", g_iPlayerPracLocationSnap[ObservedUser][g_iPlayerPracLocationSnapIdClient[ObservedUser]], (g_mapZonesTypeCount[g_iClientInZone[ObservedUser][2]][3] + 1));
							}
							else
							{
								Format(s_MapType, 64, "Stage: %i/%i", g_Stage[g_iClientInZone[ObservedUser][2]][ObservedUser], (g_mapZonesTypeCount[g_iClientInZone[ObservedUser][2]][3] + 1));
							}
						}
						else
						{
							Format(s_MapType, 64, "Stage: %i/%i", g_Stage[g_iClientInZone[ObservedUser][2]][ObservedUser], (g_mapZonesTypeCount[g_iClientInZone[ObservedUser][2]][3] + 1)); // less \t's to make lines align
						}
					}
				}
				else
				{
					Format(s_MapType, 64, "Bonus %i", g_iClientInZone[ObservedUser][2]);
				}
				...
				else if (g_iClientInZone[ObservedUser][2] == 0 && g_iCurrentStyle[ObservedUser] != 0) // Styles
				{
					if (g_fRecordStyleMapTime[style] != 9999999.0)
					{
						// fluffys
				...
				{
					if (g_iCurrentStyle[ObservedUser] == 0)
					{
						if (StrEqual(g_szBonusFastestTime[g_iClientInZone[ObservedUser][2]], "N/A"))
						{
							Format(s_MapWorldRecord, 64, "SR: %s", g_szBonusFastestTime[g_iClientInZone[ObservedUser][2]]);
						}
						else
						{	
							Format(s_MapWorldRecord, 64, "SR: %s", g_szBonusFastestTime[g_iClientInZone[ObservedUser][2]]);
					...
					else if (g_iCurrentStyle[ObservedUser] != 0) // Styles
					{
						if (StrEqual(g_szStyleBonusFastestTime[style][g_iClientInZone[ObservedUser][2]], "N/A"))
						{
							Format(s_MapWorldRecord, 64, "SR: %s", g_szStyleBonusFastestTime[style][g_iClientInZone[ObservedUser][2]]);
						}
						else
						{
							Format(s_MapWorldRecord, 64, "SR: %s", g_szStyleBonusFastestTime[style][g_iClientInZone[ObservedUser][2]]);
				...
				if (g_iClientInZone[ObservedUser][2] == 0 && style == 0)
				{
					...
					{
						if (g_fPersonalRecord[ObservedUser] > 0.0)
							Format(s_MapPersonalRecord, 64, "PB: %s", g_szPersonalRecord[ObservedUser]);
						else
							Format(s_MapPersonalRecord, 64, "PB: N/A");
					}
					else
				...
				else if (g_iClientInZone[ObservedUser][2] == 0 && g_iCurrentStyle[ObservedUser] != 0) // Styles
				{
					if (g_fRecordStyleMapTime[style] != 9999999.0)
					{
						if (g_fPersonalStyleRecord[style][ObservedUser] > 0.0)
							Format(s_MapPersonalRecord, 64, "PB: %s", g_szPersonalStyleRecord[style][ObservedUser]);
						else
							Format(s_MapPersonalRecord, 64, "PB: N/A");
					}
					else
				...
				{
					if (g_iCurrentStyle[ObservedUser] == 0)
						Format(s_MapPersonalRecord, 64, "PB: %s	", g_szPersonalRecordBonus[g_iClientInZone[ObservedUser][2]][ObservedUser]);
					else if (g_iCurrentStyle[client] != 0) // Styles
						Format(s_MapPersonalRecord, 64, "PB: %s	", g_szStylePersonalRecordBonus[style][g_iClientInZone[ObservedUser][2]][client]);
				}
				...
				if (ObservedUser == g_RecordBot)
					Format(final_string, 256, "[Map Replay]\n[%s]\n[(%s) WR : %s]", s_MapType, g_szRecordPlayer, g_szRecordMapTime);
				else if (ObservedUser == g_BonusBot)
					Format(final_string, 256, "[%s Record Replay]\n[(%s) WR : %s]", s_MapType, g_szBonusName, g_szBonusTime);
				else if (ObservedUser == g_WrcpBot)
					if (g_bManualStageReplayPlayback)
						Format(final_string, 256, "[Stage %i Replay]\n[(%s) WRCP : %s]", g_iSelectedReplayStage, g_szWrcpReplayName[g_iSelectedReplayStage], g_szWrcpReplayTime[g_iSelectedReplayStage]);
					else
			...
			else if (g_iCentreHudModule[client][i] == 2)
			{
				...
				if (GetGameTime() - g_fLastDifferenceTime[client] > 5.0)
				{
					...
					{	
						...
						if(g_iCustomCheckpointCompareType[client] == 1) {
							if (g_fRecordMapTime != 9999999.0)
							{
								// fluffys
								if (g_bPracticeMode[client])
									Format(g_szLastCustomCheckpointDifference[client], 64, "SR: <font color='#fc0'>%s</font>", g_szRecordMapTime);
								else
									Format(g_szLastCustomCheckpointDifference[client], 64, "SR: <font color='#fc0'>%s</font>", g_szRecordMapTime);
							}
							else
								Format(g_szLastCustomCheckpointDifference[client], 64, "SR: N/A");
						}
						//PB
							...
							{
								if (g_fPersonalRecord[client] > 0.0)
									Format(g_szLastCustomCheckpointDifference[client], 64, "PB: %s", g_szPersonalRecord[client]);
								else
									Format(g_szLastCustomCheckpointDifference[client], 64, "PB: N/A	");
							}
							else
								Format(g_szLastCustomCheckpointDifference[client], 64, "PB: N/A	");
						}
						//CUSTOM
							...
							switch (g_iCustomCheckpointCompareType[client]) {
								case 3 : Format(g_szLastCustomCheckpointDifference[client], 64, "R10:");
								case 4 : Format(g_szLastCustomCheckpointDifference[client], 64, "G1:");
								case 5 : Format(g_szLastCustomCheckpointDifference[client], 64, "G2:");
								case 6 : Format(g_szLastCustomCheckpointDifference[client], 64, "G3:");
								case 7 : Format(g_szLastCustomCheckpointDifference[client], 64, "G4:");
							...
							if (g_fGroupRuntimes[g_iCustomCheckpointCompareType[client]-3] > 0.0){
								...
								Format(g_szLastCustomCheckpointDifference[client], 64, "%s N/A", g_szLastCustomCheckpointDifference[client]);
						}
					}
					else if (g_iClientInZone[client][2] == 0 && g_iCurrentStyle[client] != 0) // Styles
					{
						if (g_fRecordStyleMapTime[style] != 9999999.0)
						{
							// fluffys
							if (g_bPracticeMode[client])
								Format(g_szLastCustomCheckpointDifference[client], 64, "SR: <font color='#fc0'>%s</font>", g_szRecordStyleMapTime[style]);
							else
								Format(g_szLastCustomCheckpointDifference[client], 64, "SR: <font color='#fc0'>%s</font>", g_szRecordStyleMapTime[style]);
						}
						else
					...
					else
					{
						if (g_iCurrentStyle[client] == 0)
						{
							if (StrEqual(g_szLastCustomCheckpointDifference[g_iClientInZone[client][2]], "N/A"))
							{
								Format(g_szLastCustomCheckpointDifference[client], 64, "SR: <font color='#fff'>%s</font>", g_szBonusFastestTime[g_iClientInZone[client][2]]);
							}
							else
							{	
								Format(g_szLastCustomCheckpointDifference[client], 64, "SR: <font color='#fc0'>%s</font>", g_szBonusFastestTime[g_iClientInZone[client][2]]);
							}
						}
						else if (g_iCurrentStyle[client] != 0) // Styles
						{
							if (StrEqual(g_szStyleBonusFastestTime[style][g_iClientInZone[client][2]], "N/A"))
							{
								Format(g_szLastCustomCheckpointDifference[client], 64, "SR: <font color='#fff'>%s</font>", g_szStyleBonusFastestTime[style][g_iClientInZone[client][2]]);
							}
							else
							{
								Format(g_szLastCustomCheckpointDifference[client], 64, "SR: <font color='#fc0'>%s</font>", g_szStyleBonusFastestTime[style][g_iClientInZone[client][2]]);
							}
						}
					}
				}
				Format(module[i], 128, "%s", g_szLastCustomCheckpointDifference[client]);
			}
...
public void Checkpoint(int client, int zone, int zonegroup, float time, float speed)
{
	...
	//CURRRENT RUN CP SPEED
	g_fCheckpointSpeedsNew[client][zone] = speed;

	// Server record time difference
	char sz_srDiff[128];
	...
	//speed difference
	char szSpeed[32];
	char sz_srSpeedDiff[128];
	char sz_srSpeedDiff_colorless[128];
	...
	if(g_iCustomCheckpointCompareType[client] > 2){
		...
		char szGroup[8];
		switch (g_iCustomCheckpointCompareType[client]) {
			case 3: Format(szGroup, sizeof szGroup, "%s", "R10");
			case 4: Format(szGroup, sizeof szGroup, "%s", "G1");
			case 5: Format(szGroup, sizeof szGroup, "%s", "G2");
			case 6: Format(szGroup, sizeof szGroup, "%s", "G3");
			case 7: Format(szGroup, sizeof szGroup, "%s", "G4");
			case 8: Format(szGroup, sizeof szGroup, "%s", "G5");
		}

		//CHAT MESSAGE FORMATTING
		if (f_srDiff != 666666.0){
			FormatTimeFloat(client, f_srDiff, 3, sz_srDiff, 128);

			if (f_srDiff > 0){
				Format(sz_srDiff_colorless, 128, "-%s", sz_srDiff);
				Format(sz_srDiff, 128, "%c%s: %c-%s%c", WHITE, szGroup, LIGHTGREEN, sz_srDiff, WHITE);
			}
			else {
				Format(sz_srDiff_colorless, 128, "+%s", sz_srDiff);
				Format(sz_srDiff, 128, "%c%s: %c+%s%c", WHITE, szGroup, RED, sz_srDiff, WHITE);
			}
		}
		else
			Format(sz_srDiff, sizeof sz_srDiff, "%s: N/A", szGroup);

		//CENTER HUD FORMATTING
		if(g_iCustomCheckpointCompareType[client] == 3)
			if(g_bTOP){
				if (f_srDiff > 0)
					Format(g_szLastCustomCheckpointDifference[client], 64, "R10: <font color='#5e5'>%s</font>", sz_srDiff_colorless);
				else
					Format(g_szLastCustomCheckpointDifference[client], 64, "R10: <font color='#f32'>%s</font>", sz_srDiff_colorless);
			}
			else
				Format(g_szLastCustomCheckpointDifference[client], 64, "R10: N/A");
		else if(g_iCustomCheckpointCompareType[client] == 4)
			if(g_bG1){
				if (f_srDiff > 0)
					Format(g_szLastCustomCheckpointDifference[client], 64, "G1: <font color='#5e5'>%s</font>", sz_srDiff_colorless);
				else
					Format(g_szLastCustomCheckpointDifference[client], 64, "G1: <font color='#f32'>%s</font>", sz_srDiff_colorless);
			}
			else
				Format(g_szLastCustomCheckpointDifference[client], 64, "G1: N/A", sz_srDiff_colorless);
		else if(g_iCustomCheckpointCompareType[client] == 5)
			if(g_bG2){
				if (f_srDiff > 0)
					Format(g_szLastCustomCheckpointDifference[client], 64, "G2: <font color='#5e5'>%s</font>", sz_srDiff_colorless);
				else
					Format(g_szLastCustomCheckpointDifference[client], 64, "G2: <font color='#f32'>%s</font>", sz_srDiff_colorless);
			}
			else
				Format(g_szLastCustomCheckpointDifference[client], 64, "G2: N/A", sz_srDiff_colorless);
		else if(g_iCustomCheckpointCompareType[client] == 6)
			if(g_bG3){
				if (f_srDiff > 0)
					Format(g_szLastCustomCheckpointDifference[client], 64, "G3: <font color='#5e5'>%s</font>", sz_srDiff_colorless);
				else
					Format(g_szLastCustomCheckpointDifference[client], 64, "G3: <font color='#f32'>%s</font>", sz_srDiff_colorless);
			}
			else
				Format(g_szLastCustomCheckpointDifference[client], 64, "G3: N/A", sz_srDiff_colorless);
		else if(g_iCustomCheckpointCompareType[client] == 7)
			if(g_bG4){
				if (f_srDiff > 0)
					Format(g_szLastCustomCheckpointDifference[client], 64, "G4: <font color='#5e5'>%s</font>", sz_srDiff_colorless);
				else
					Format(g_szLastCustomCheckpointDifference[client], 64, "G4: <font color='#f32'>%s</font>", sz_srDiff_colorless);
			}
			else
				Format(g_szLastCustomCheckpointDifference[client], 64, "G4: N/A", sz_srDiff_colorless);
		else if(g_iCustomCheckpointCompareType[client] == 8)
			if(g_bG5){
				if (f_srDiff > 0)
					Format(g_szLastCustomCheckpointDifference[client], 64, "G5: <font color='#5e5'>%s</font>", sz_srDiff_colorless);
				else
					Format(g_szLastCustomCheckpointDifference[client], 64, "G5: <font color='#f32'>%s</font>", sz_srDiff_colorless);
			}
			else
				Format(g_szLastCustomCheckpointDifference[client], 64, "G5: N/A", sz_srDiff_colorless);
...
/////
	...
	//PB
	if (g_bCheckpointSpeedsFound[client] && g_fCheckpointSpeedsRecord[client][zone] > 0.0 && g_bTimerRunning[client])
	{
		f_SpeedDiff = (g_fCheckpointSpeedsRecord[client][zone] - speed);
	...
	if (g_iCustomCheckpointCompareType[client] <= 2) {
		if (g_bCheckpointRecordFound[zonegroup] && g_fCheckpointServerRecord[zonegroup][zone] > 0.0 && g_bTimerRunning[client])
		{
			...
			if (f_srDiff > 0)
			{
				...
				Format(g_szLastCustomCheckpointDifference[client], 128, "SR: <font color='#5e5'>%s</font>", sz_srDiff_colorless);
				if (zonegroup > 0)
			...
			else
			{
				...
				Format(g_szLastCustomCheckpointDifference[client], 128, "SR: <font color='#f32'>%s</font>", sz_srDiff_colorless);
				if (zonegroup > 0)
			...
			Format(g_szLastCustomCheckpointDifference[client], 128, "SR: N/A");
		}
	}
	...
	//FORMAT MESSAGES WITH PB VALUES
	...
	if (g_bCheckpointsFound[zonegroup][client] && g_bTimerRunning[client] && g_fCheckpointTimesRecord[zonegroup][client][zone] > 0.1)
	{
		...
		diff = ( g_fCheckpointTimesRecord[zonegroup][client][zone] - time);
		...
		if(g_iCustomCheckpointCompareType[client] == 2){
			Format(sz_srDiff, sizeof sz_srDiff, "PB: %s", szDiff);
			Format(g_szLastCustomCheckpointDifference[client], sizeof g_szLastCustomCheckpointDifference, "%s", g_szLastPBDifference[client]);
		}
		...
		Format(szSpecMessage, sizeof(szSpecMessage), "%t", "Misc31", g_szChatPrefix, szName, g_iClientInZone[client][1] + 1, szTime, szDiff, sz_srDiff, szSpeed, sz_SpeedDiff, sz_srSpeedDiff);
	...
	// if first run
		...
		if (percent > -1.0)
		{
			...
			Format(szSpecMessage, sizeof(szSpecMessage), "%t", "Misc33", g_szChatPrefix, szName, g_iClientInZone[client][1] + 1, szTime, sz_srDiff, szSpeed, sz_SpeedDiff, sz_srSpeedDiff);
			CheckpointToSpec(client, szSpecMessage);
		}
	}
}
