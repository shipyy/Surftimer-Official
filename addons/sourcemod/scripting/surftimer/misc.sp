...
public void SetClientDefaults(int client)
{
	...
	Format(g_szLastCustomCheckpointDifferenceMHUD[client], 64, "");
...
public void MinimalHudAlive(int client){
	...
	if(g_bMinimalHUD[client]){
		...
		if ( (GetGameTime() - g_fLastDifferenceTime[client] <= 5.0) && g_bTimerRunning[client] && g_iCurrentStyle[client] == 0)
			Format(s_Timer, 64, "%s\n%s", s_Timer, g_szLastCustomCheckpointDifferenceMHUD[client]);
...
//////
	...
	if(g_bCustomGroupExists[g_iCustomCheckpointsType[client] - 1][zonegroup] && g_bTimerRunning[client]){
		...
		//FORMAT MINIMAL HUD
		Format(g_szLastCustomCheckpointDifferenceMHUD[client], 64, "[%s: %s]", szGroup, CustomCheckpointDifference_colorless);
	...
	//custom selected group doesnt exist
		...
		Format(g_szLastCustomCheckpointDifferenceMHUD[client], 64, "[%s: N/A]", szGroup);
